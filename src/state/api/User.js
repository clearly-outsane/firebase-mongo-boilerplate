...
const createUser = (state, dispatch, body, cb) => {
  ...
  if (!body.uid)
    emailSignUp(state, dispatch, body.email, body.password, createMongoUser);
  else createMongoUser({ uid: body.uid }, true);
};
...
const signInUser = (type = "email", state, dispatch, body, cb) => {
  ...
  if (type === "email")
    emailSignIn(state, dispatch, body.email, body.password, getMongoUser);
