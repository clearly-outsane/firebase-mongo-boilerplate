...
const AlbumList = () => {
  ...
  useEffect(() => {
    if ((!state.user || !state.user.albums) && !state.initialLoading) { setLoading(false) }
    return () => {}
          ...
          <Table className={classes.table} aria-label='customized table'>
            <TableBody>
              {singles.map((singleId) => {
                const foundSingle = state.singlesArray.find(
                  (s) => s._id === singleId
                )
                if (foundSingle) { return <SingleRow key={foundSingle.name} row={foundSingle} /> }
              })}
            </TableBody>
          </Table>
        </TableContainer>
